-- Update get_token_mints() to return launch_status + chain
...
RETURNS TABLE (
  ...
  dev_wallet_id        uuid,          -- ← added
  vanity_keypair_id    uuid,          -- ← added
  ...
  IF (SELECT public.is_super_admin()) THEN
    RETURN QUERY
      SELECT
        ...
        tm.logo_url, tm.decimals, tm.token_type,
        tm.chain,                     -- ← added
        tm.launch_status,             -- ← added
        tm.current_supply, tm.max_supply,
        tm.metadata_uri, tm.is_active,
        tm.authority_revoked, tm.freeze_revoked,
        tm.dev_wallet_id,             -- ← added
        tm.vanity_keypair_id,         -- ← added
