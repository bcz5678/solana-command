DROP FUNCTION IF EXISTS public.update_token_draft(uuid,text,text,text,smallint,text,numeric,text,text,uuid);
...
CREATE OR REPLACE FUNCTION public.update_token_draft(
  ...
  p_tiktok_url           text      DEFAULT NULL,   -- ← added
  p_instagram_url        text      DEFAULT NULL,   -- ← added
  p_discord_url          text      DEFAULT NULL,   -- ← added
  p_communities_url      text      DEFAULT NULL,   -- ← added
...
BEGIN
  ...
  -- Validate dev wallet if provided
  ...
  -- Apply updates
  -- COALESCE keeps existing value if NULL.
  -- Empty string '' explicitly clears a social URL (NULLIF converts '' → NULL).
...
REVOKE ALL    ON FUNCTION public.update_token_draft(uuid,text,text,text,text,text,text,text,text,smallint,text,numeric,text,text,uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.update_token_draft(uuid,text,text,text,text,text,text,text,text,smallint,text,numeric,text,text,uuid) TO authenticated;