-- ============================================================
-- Fix 1: Remove service_role gate from private function
--        Replace with DB-only super admin check
-- ============================================================
...
DECLARE
  ...
  v_vault_name      text;
BEGIN
  -- ── Auth: DB-only super admin check ───────────────────────
  -- Does not depend on JWT claims or role —
  -- works correctly inside SECURITY DEFINER context
  IF NOT EXISTS (
  ...
  WHERE  vk.status = 'available'
    ...
    AND  (p_vanity_prefix IS NULL OR vk.vanity_prefix = p_vanity_prefix)
...
-- ============================================================
-- Fix 2: Public wrapper — DB-only check, same pattern
-- ============================================================
...
AS $$
BEGIN
  -- DB-only check — reliable in all JWT contexts
  IF NOT EXISTS (
    SELECT 1 FROM private.super_admins
    WHERE  user_id    = auth.uid()
      AND  revoked_at IS NULL
  ...
  RETURN QUERY
    ...
    FROM private.claim_vanity_keypair(
      ...
      p_vanity_prefix,
      p_mint_id,
      COALESCE(p_claimed_by, auth.uid())
    );
END;
$$;
