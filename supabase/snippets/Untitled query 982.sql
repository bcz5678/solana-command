CREATE OR REPLACE FUNCTION public.build_token_draft(
  ...
  p_website_url          text      DEFAULT NULL,   -- ← added
  p_twitter_url          text      DEFAULT NULL,   -- ← added
  p_telegram_handle      text      DEFAULT NULL,   -- ← added
...
BEGIN
  ...
  VALUES (
    ...
    p_description,
    p_logo_url,
    NULLIF(p_website_url,     ''),   -- ← added, '' → NULL
    NULLIF(p_twitter_url,     ''),   -- ← added
    NULLIF(p_telegram_handle, ''),   -- ← added
    NULLIF(p_tiktok_url,      ''),
    NULLIF(p_instagram_url,   ''),
    NULLIF(p_discord_url,     ''),
    NULLIF(p_communities_url, ''),
    p_decimals,
    p_token_type,
    p_max_supply,
    p_metadata_uri,
...
REVOKE ALL    ON FUNCTION public.build_token_draft(uuid,uuid,text,text,text,text,text,text,text,text,text,text,text,smallint,text,numeric,text,text,text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.build_token_draft(uuid,uuid,text,text,text,text,text,text,text,text,text,text,text,smallint,text,numeric,text,text,text) TO authenticated;