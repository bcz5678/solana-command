...
export default function ConsolidateForm() {
    ...
    // Re-fires ONE sender's sweep in place — same route, a single-item
    // senders array. Keyed by walletId so it can't disturb any other row's
    // status, including another retry running concurrently.
    async function retrySender(walletId: string) {
        if (!activeTransfer) return
        setSenderStatuses(prev => ({ ...prev, [walletId]: 'loading' }))
        ...
        try {
            const res = await fetch('/api/wallet/transfer/consolidate', {
                ...
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    receiverPublicKey: activeTransfer.receiverPublicKey,
                    senders: [{ walletId }],
                }),
            })
            if (res.ok) {
        ...
        setSenderStatuses(prev => ({ ...prev, [walletId]: success ? 'success' : 'error' }))
    }
            ...
            <Dialog open={showProgress} onOpenChange={open => { if (!open && transfersDone) resetAfterTransfer() }}>
                <DialogContent className="max-w-md">
                                ...
                                <div key={s.walletId} className="flex items-center gap-3 py-3">
                                    ...
                                    {status === 'error' && (
                                        <Button
                                            ...
                                            onClick={() => retrySender(s.walletId)}
                                        >
                                            Retry
                                        </Button>
                                    )}
                    ...
                    {transfersDone && (() => {
                        ...
                        const failed  = Object.values(senderStatuses).filter(s => s === 'error').length
                        return (
                            <>
                                <div className={[
