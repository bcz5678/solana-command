...
export function initializeConnection(): Connection {
  ...
  return new Solana({ endpointUrl: process.env.SOLANA_RPC_URL }).connection;
}

export function initializeQuickNodeSolana(): Solana {
  if (!process.env.SOLANA_RPC_URL) {
    throw new Error('SOLANA_RPC_URL is not set');
  }
  return new Solana({ endpointUrl: process.env.SOLANA_RPC_URL });
}
